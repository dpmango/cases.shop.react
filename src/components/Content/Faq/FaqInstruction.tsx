...
const FaqInstruction = () => {
  const { faqInfo } = useAppSelector((state) => state.sessionState)
        ...
        {faqInfo.map((x, idx) => (
          <SwiperSlide key={idx}>
            <div className="faq__slide">
              <img src={x[0]} alt="" className="faq__image" />
              <p className="faq__text p-small" dangerouslySetInnerHTML={{ __html: x[1] }} />
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
